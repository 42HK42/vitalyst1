"""API routes for the Vitalyst Knowledge Graph."""
...
from src.services.ai.ai import AIService
from src.services.graph.neo4j import Neo4jService
from src.utils.helpers.auth import get_current_user
...
async def create_node(node: NodeCreate, current_user=Depends(get_current_user)):
    """Create a new node."""
    return await neo4j.create_node(node)


@router.get("/nodes/{node_id}", response_model=Node)
async def get_node(node_id: str, current_user=Depends(get_current_user)):
...
async def update_node(
    node_id: str, node: NodeUpdate, current_user=Depends(get_current_user)
):
...
async def delete_node(node_id: str, current_user=Depends(get_current_user)):
...
async def enrich_node(node_id: str, current_user=Depends(get_current_user)):
